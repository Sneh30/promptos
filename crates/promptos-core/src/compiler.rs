...
impl Compiler {
    ...
    pub async fn compile(&mut self, input: &str, profile: Option<&ModelProfileData>) -> Result<CompilationResult, String> {
        ...
        let annotations = analyzer.analyze(&mut root).await.map_err(|e| format!("Analysis error: {}", e))?;

        let mut diagnostics = DiagnosticBuilder::new();
        for diag in &annotations.diagnostics {
            match diag.severity {
                DiagnosticSeverity::Error => diagnostics.error(&diag.diagnostic_code, &diag.message, diag.span, diag.recommendation.as_deref()),
                DiagnosticSeverity::Warning => diagnostics.warning(&diag.diagnostic_code, &diag.message, diag.span, diag.recommendation.as_deref()),
                DiagnosticSeverity::Suggestion => diagnostics.suggestion(&diag.diagnostic_code, &diag.message, diag.span, diag.recommendation.as_deref()),
        ...
        let result = CompilationResult {
            ...
            metrics: CompilationMetrics {
                ...
                passes_applied: pass_results.iter().filter(|r| r.applied).map(|r| r.pass_name.clone()).collect(),
                compilation_time_ms,
            },
