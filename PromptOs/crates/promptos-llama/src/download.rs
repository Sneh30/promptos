...
impl ModelDownloader {
    pub fn new() -> Self {
        ...
        let dir = PathBuf::from(format!(
            "{}/Library/Application Support/com.promptos.app/models",
            home
        ));
    ...
    pub fn download_default_model(&self) -> Result<String, String> {
        ...
        if dest.exists() {
            info!(
                "Download — default model already exists at {}",
                dest.display()
            );
    ...
    pub fn download_model(&self, url: &str, dest: &Path) -> Result<String, String> {
        info!(
            "Download — url={}, dest={}, content_length=unknown",
            url,
            dest.display()
        );
        ...
        std::io::Write::write_all(&mut file, &bytes).map_err(|e| format!("Write error: {}", e))?;
        let downloaded = bytes.len() as u64;

        info!(
            "Download — bytes_received={}, total={}, path={}",
            downloaded,
            total,
            dest.display()
        );
