    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            if let metrics = viewModel.metrics {
                RiskRow(label: "Hallucination Risk", value: Double(metrics.hallucination_risk), format: .percentage)
                RiskRow(label: "Quality Score", value: Double(metrics.quality_score), format: .score)
